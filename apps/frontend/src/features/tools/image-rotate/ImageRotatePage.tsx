...
type AngleOption = { label: string; value: number }
...
export default function ImageRotatePage() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const isOdd = angle === 90 || angle === -90 || angle === 270
      ...
      setResult({ url, name: `rotated_${file.name.replace(/\.[^.]+$/, '.png')}`, size: blob.size })
    } catch {
  ...
  return (
    <main className="min-h-screen bg-background">
      <div className="container mx-auto max-w-4xl px-4 py-8">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-foreground">{tool?.title ?? 'Rotate Image'}</h1>
          <p className="mt-1 text-sm text-foreground-muted">{tool?.shortDescription ?? 'Rotate images by 90°, 180°, or 270°.'}</p>
        </div>
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            {result ? (
              <div className="flex flex-col h-full">
                <img src={result.url} alt="Rotated result" className="w-full object-contain max-h-80" />
              ...
              <div className="flex flex-col items-center justify-center h-full min-h-48 p-8 text-center">
                <p className="text-sm text-foreground-muted">Rotated image will appear here</p>
              </div>
            )}
          </div>
        </div>
